...
# Zurich.
"""Applies rigid, affine and/or b-spline registration to (3D) images using elastix."""
...
def apply_registration_elastix(
    *,
    ...
    # Check if zarr_url is not reference_zarr_url otherwise stop
    if zarr_url == reference_zarr_url:
        logger.info(
            f"Skipping registration for {zarr_url} as it is the reference "
            "acquisition."
        )
        return

    ROI_table_ref_path = f"{reference_zarr_url}/tables/{roi_table}"
    ROI_table_ref = ad.read_zarr(ROI_table_ref_path)
...
def write_registered_zarr(
    ...
    for i, roi_indices in enumerate(list_indices):
        ...
        # Get base_url in case zarr_url is path to label image
        if Path(zarr_url).parent.name == "labels":
            base_url = str(Path(zarr_url).parent.parent)
        else:
            base_url = zarr_url

        fn_pattern = f"{roi_table_name}_roi_{i}_t*.txt"
        parameter_path = Path(base_url) / "registration"
        ...
        if axes_list == ["c", "z", "y", "x"]:
            ...
            for ind_ch in range(num_channels):
                idx = (slice(ind_ch, ind_ch + 1), *reference_region)
                img = load_region(
                    data_zyx=data_array[ind_ch], region=region, compute=True
                )
                itk_img = to_itk(np.squeeze(img), scale=tuple(pxl_sizes_zyx))
                parameter_object_adapted = adapt_itk_params(
                    parameter_object=parameter_object, itk_img=itk_img
                )
                registered_img = apply_transform(itk_img, parameter_object_adapted)
                new_array[idx] = np.expand_dims(to_numpy(registered_img), axis=0)
        elif axes_list == ["z", "y", "x"]:
            img = load_region(data_zyx=data_array, region=region, compute=True)
            itk_img = to_itk(img, scale=tuple(pxl_sizes_zyx))
            parameter_object_adapted = adapt_itk_params(
                parameter_object=parameter_object, itk_img=itk_img
            )
            registered_img = apply_transform(itk_img, parameter_object_adapted)
            new_array[reference_region] = to_numpy(registered_img)
